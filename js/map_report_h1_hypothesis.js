/**
 * map_report_h1_hypothesis.js
 * Visualizes H1 Hypothesis:
 * - Default Map: Clean (CartoDB)
 * - Markers: Group Familiarity (Blue/Red)
...
function initializeMap(data) {
    ...
    // Group Colors (For the Dots)
    var colorLess = "#3498db"; // Blue
    var colorMore = "#e74c3c"; // Red
    ...
    // Initialize Map with CLEAN layer as default
    var map = L.map('map_full', {
        center: [47.0, 19.0],
        zoom: 7,
        layers: [cleanLayer] // <--- CHANGED TO DEFAULT
    });
    ...
    data.forEach(function(row) {
        ...
        var group = row.drone_familiarity_group; 
        ...
        if (!isNaN(attitude) && !isNaN(lat) && !isNaN(lng)) {
            ...
            if (group === "Less") groupLess.push(attitude);
            else if (group === "More") groupMore.push(attitude);
        }
    });
    ...
    // B. Markers (Colored by Group)
    formattedData.forEach(function(d) {
        var color = (d.group === "More") ? colorMore : colorLess;
    ...
    // C. Legend
    addLegend(map, colorLess, colorMore);

    // --- 5. RUN STATISTICS ---
    if (groupMore.length > 1 && groupLess.length > 1) {
        var stats = calculateIndependentTTest(groupMore, groupLess);
        displayStats(stats);
    } else {
 ...
 * Legend
 */
function addLegend(map, colorLess, colorMore) {
    ...
    legend.onAdd = function (map) {
        ...
        div.innerHTML = `
            <div style="font-weight: bold; border-bottom: 1px solid #eee; margin-bottom:5px;">Familiarity (Dots)</div>
            <div><span style="background:${colorLess}; width:10px; height:10px; display:inline-block; border-radius:50%; margin-right:5px;"></span>Less Familiar</div>
            <div style="margin-bottom:10px;"><span style="background:${colorMore}; width:10px; height:10px; display:inline-block; border-radius:50%; margin-right:5px;"></span>More Familiar</div>
...
function calculateIndependentTTest(sample1, sample2) {
    ...
    return { meanMore: mean1, meanLess: mean2, t: tValue, df: df, p: pValue, sig: pValue < 0.05 };
}
...
function displayStats(stats) {
    ...
    var sigText = stats.sig ? "<b>Supported.</b> Significant difference." : "<b>Not Supported.</b> No significant difference.";

    var html = `
        <table style="width:100%; text-align: left; font-family: sans-serif; font-size: 13px;">
            <tr><td><b>More Familiar (Mean):</b></td><td>${stats.meanMore.toFixed(3)}</td></tr>
            <tr><td><b>Less Familiar (Mean):</b></td><td>${stats.meanLess.toFixed(3)}</td></tr>
